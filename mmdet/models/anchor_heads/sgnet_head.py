...
class SGNetHead(nn.Module):
    ...
    def _init_layers(self):
        ...
        self.sgnet_cof_1_1 = nn.Conv2d(self.feat_channels, self.nc * 1, 3, padding=1)
        self.sgnet_cof_2_2 = nn.Conv2d(self.feat_channels, self.nc * 4, 3, padding=1)
        self.sgnet_cof_3_3 = nn.Conv2d(self.feat_channels, self.nc * 9, 3, padding=1)
        self.sgnet_cof_4_4 = nn.Conv2d(self.feat_channels, self.nc * 16, 3, padding=1)

        self.sgnet_lat = nn.Conv2d(512, self.nc, 3, padding=1)
        self.sgnet_lat0 = nn.Conv2d(768, 512, 1, padding=0)
        self.relu = nn.ReLU(inplace=True)

        self.crop_cuda_1_1 = CropSplit(1)
        self.crop_gt_cuda_1_1 = CropSplitGt(1)
        self.crop_cuda_2_2 = CropSplit(2)
        self.crop_gt_cuda_2_2 = CropSplitGt(2)
        self.crop_cuda_3_3 = CropSplit(3)
        self.crop_gt_cuda_3_3 = CropSplitGt(3)
        self.crop_cuda_4_4 = CropSplit(4)
        self.crop_gt_cuda_4_4 = CropSplitGt(4)
        ...
        self.sgnet_track = nn.Conv2d(self.feat_channels * 3, 512, 1, padding=0)
    ...
    def init_weights(self):
        ...
        normal_init(self.sgnet_cof_1_1, std=0.01)
        normal_init(self.sgnet_cof_2_2, std=0.01)
        normal_init(self.sgnet_cof_3_3, std=0.01)
        normal_init(self.sgnet_cof_4_4, std=0.01)

        normal_init(self.sgnet_lat, std=0.01)
        normal_init(self.sgnet_lat0, std=0.01)
    ...
    def forward(self, feats, feats_x, flag_train=True):
        ...
        for x, x_f, scale, stride in zip(feats, feats_x, self.scales, self.strides):
            ...
            ########COFFECIENTS###############
            cof_pred_1_1 = self.sgnet_cof_1_1(cls_feat)
            cof_pred_2_2 = self.sgnet_cof_2_2(cls_feat)
            cof_pred_3_3 = self.sgnet_cof_3_3(cls_feat)
            cof_pred_4_4 = self.sgnet_cof_4_4(cls_feat)

            cof_preds_1_1.append(cof_pred_1_1)
            cof_preds_2_2.append(cof_pred_2_2)
            cof_preds_3_3.append(cof_pred_3_3)
            cof_preds_4_4.append(cof_pred_4_4)
            # cof_preds.append(cof_pred)
            cof_preds = cof_preds_1_1 + cof_preds_2_2 + cof_preds_3_3 + cof_preds_4_4
        ...
        feat_masks = self.relu(self.sgnet_lat(self.relu(self.sgnet_lat0(feat_masks))))
        ...
        track_feats = self.sgnet_track(track_feats)

        if flag_train:
            track_feats_ref = torch.cat(track_feats_ref, dim=1)
            track_feats_ref = self.sgnet_track(track_feats_ref)
    ...
    def loss(self,
        ...
        flatten_cof_preds_1_1 = []
        flatten_cof_preds_2_2 = []
        flatten_cof_preds_3_3 = []
        flatten_cof_preds_4_4 = []

        for ii in range(int(len(cof_preds) / 4)):
            flatten_cof_preds_1_1.append(cof_preds[ii].permute(0, 2, 3, 1).reshape(cof_preds[ii].shape[0], -1, 32 * 1))

        for ii in range(int(len(cof_preds) / 4), int(len(cof_preds) / 2)):
            flatten_cof_preds_2_2.append(cof_preds[ii].permute(0, 2, 3, 1).reshape(cof_preds[ii].shape[0], -1, 32 * 4))

        for ii in range(int(len(cof_preds) / 2), int(len(cof_preds) * 3 / 4)):
            flatten_cof_preds_3_3.append(cof_preds[ii].permute(0, 2, 3, 1).reshape(cof_preds[ii].shape[0], -1, 32 * 9))

        for ii in range(int(len(cof_preds) * 3 / 4), int(len(cof_preds))):
            flatten_cof_preds_4_4.append(cof_preds[ii].permute(0, 2, 3, 1).reshape(cof_preds[ii].shape[0], -1, 32 * 16))

        # flatten_cof_preds = [
        #     cof_pred.permute(0, 2, 3, 1).reshape(cof_pred.shape[0], -1, 32 * 16)
        #     for cof_pred in cof_preds
        # ]
        ...
        flatten_cof_preds_1_1 = torch.cat(flatten_cof_preds_1_1, dim=1)
        flatten_cof_preds_2_2 = torch.cat(flatten_cof_preds_2_2, dim=1)
        flatten_cof_preds_3_3 = torch.cat(flatten_cof_preds_3_3, dim=1)
        flatten_cof_preds_4_4 = torch.cat(flatten_cof_preds_4_4, dim=1)
        ...
        for i in range(num_imgs):
            ...
            cof_pred_1_1 = flatten_cof_preds_1_1[i][pos_inds]
            cof_pred_2_2 = flatten_cof_preds_2_2[i][pos_inds]
            cof_pred_3_3 = flatten_cof_preds_3_3[i][pos_inds]
            cof_pred_4_4 = flatten_cof_preds_4_4[i][pos_inds]
            ...
            avg_area = torch.mean(area).cpu().numpy()

            if avg_area > 1000:
                cof_pred = cof_pred_4_4
            elif avg_area > 500:
                cof_pred = cof_pred_3_3
            # elif avg_area > 200:
            #     cof_pred = cof_pred_2_2
            else:
                cof_pred = cof_pred_2_2
            ...
            # print(area.shape, img_mask1.shape, cof_pred_1_1.shape, cof_pred_2_2.shape, cof_pred_3_3.shape, cof_pred_4_4.shape)
            if avg_area > 1000:
                pos_masks00 = torch.sigmoid(img_mask1 @ cof_pred[:, 0:32].t())
                pos_masks01 = torch.sigmoid(img_mask1 @ cof_pred[:, 32:64].t())
                pos_masks02 = torch.sigmoid(img_mask1 @ cof_pred[:, 64:96].t())
                pos_masks03 = torch.sigmoid(img_mask1 @ cof_pred[:, 96:128].t())
                pos_masks10 = torch.sigmoid(img_mask1 @ cof_pred[:, 128:160].t())
                pos_masks11 = torch.sigmoid(img_mask1 @ cof_pred[:, 160:192].t())
                pos_masks12 = torch.sigmoid(img_mask1 @ cof_pred[:, 192:224].t())
                pos_masks13 = torch.sigmoid(img_mask1 @ cof_pred[:, 224:256].t())
                pos_masks20 = torch.sigmoid(img_mask1 @ cof_pred[:, 256:288].t())
                pos_masks21 = torch.sigmoid(img_mask1 @ cof_pred[:, 288:320].t())
                pos_masks22 = torch.sigmoid(img_mask1 @ cof_pred[:, 320:352].t())
                pos_masks23 = torch.sigmoid(img_mask1 @ cof_pred[:, 352:384].t())
                pos_masks30 = torch.sigmoid(img_mask1 @ cof_pred[:, 384:416].t())
                pos_masks31 = torch.sigmoid(img_mask1 @ cof_pred[:, 416:448].t())
                pos_masks32 = torch.sigmoid(img_mask1 @ cof_pred[:, 448:480].t())
                pos_masks33 = torch.sigmoid(img_mask1 @ cof_pred[:, 480:512].t())

                pred_masks = torch.stack([pos_masks00, pos_masks01, pos_masks02, pos_masks03,
                                          pos_masks10, pos_masks11, pos_masks12, pos_masks13,
                                          pos_masks20, pos_masks21, pos_masks22, pos_masks23,
                                          pos_masks30, pos_masks31, pos_masks32, pos_masks33], dim=0)
                pred_masks = self.crop_cuda_4_4(pred_masks, bbox_dt)
                gt_mask_crop = self.crop_gt_cuda_4_4(gt_mask_new, bbox_dt)
            elif avg_area > 500:
                pos_masks00 = torch.sigmoid(img_mask1 @ cof_pred[:, 0:32].t())
                pos_masks01 = torch.sigmoid(img_mask1 @ cof_pred[:, 32:64].t())
                pos_masks02 = torch.sigmoid(img_mask1 @ cof_pred[:, 64:96].t())
                pos_masks10 = torch.sigmoid(img_mask1 @ cof_pred[:, 96:128].t())
                pos_masks11 = torch.sigmoid(img_mask1 @ cof_pred[:, 128:160].t())
                pos_masks12 = torch.sigmoid(img_mask1 @ cof_pred[:, 160:192].t())
                pos_masks20 = torch.sigmoid(img_mask1 @ cof_pred[:, 192:224].t())
                pos_masks21 = torch.sigmoid(img_mask1 @ cof_pred[:, 224:256].t())
                pos_masks22 = torch.sigmoid(img_mask1 @ cof_pred[:, 256:288].t())

                pred_masks = torch.stack([pos_masks00, pos_masks01, pos_masks02,
                                          pos_masks10, pos_masks11, pos_masks12,
                                          pos_masks20, pos_masks21, pos_masks22], dim=0)
                pred_masks = self.crop_cuda_3_3(pred_masks, bbox_dt)
                gt_mask_crop = self.crop_gt_cuda_3_3(gt_mask_new, bbox_dt)
            # # elif avg_area > 200:
            else:
                pos_masks00 = torch.sigmoid(img_mask1 @ cof_pred[:, 0:32].t())
                pos_masks01 = torch.sigmoid(img_mask1 @ cof_pred[:, 32:64].t())
                ...
                pred_masks = self.crop_cuda_2_2(pred_masks, bbox_dt)
                gt_mask_crop = self.crop_gt_cuda_2_2(gt_mask_new, bbox_dt)
            # else:
            #     pred_masks = torch.sigmoid(img_mask1 @ cof_pred[:, 0:32].t())
            #     pred_masks = self.crop_cuda_1_1(pred_masks, bbox_dt)
            #     gt_mask_crop = self.crop_gt_cuda_1_1(gt_mask_new, bbox_dt)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            for ii in range(int(len(cof_preds) / 4)):
                cof_pred_list_1_1.append(cof_preds[ii][img_id].detach())

            for ii in range(int(len(cof_preds) / 4), int(len(cof_preds) / 2)):
                cof_pred_list_2_2.append(cof_preds[ii][img_id].detach())

            for ii in range(int(len(cof_preds) / 2), int(len(cof_preds) * 3 / 4)):
                cof_pred_list_3_3.append(cof_preds[ii][img_id].detach())

            for ii in range(int(len(cof_preds) * 3 / 4), int(len(cof_preds))):
                cof_pred_list_4_4.append(cof_preds[ii][img_id].detach())

            # cof_pred_list = cof_pred_list_1_1 + cof_pred_list_2_2 + cof_pred_list_3_3 + cof_pred_list_4_4
            ...
            det_bboxes = self.get_bboxes_single(cls_score_list, bbox_pred_list,
                                                centerness_pred_list, cof_pred_list_2_2,
                                                cof_pred_list_3_3, cof_pred_list_4_4, feat_mask_list,
    ...
    def get_bboxes_single(self,
        ...
        mlvl_centerness = []
        mlvl_cofs = []
        for cls_score, bbox_pred, cof_pred_2_2, cof_pred_3_3, cof_pred_4_4, centerness, points in zip(
                cls_scores, bbox_preds, cof_preds_2_2, cof_preds_3_3, cof_preds_4_4, centernesses, mlvl_points):
            ...
            if avg_area > 1000:
                cof_pred = cof_pred_4_4.permute(1, 2, 0).reshape(-1, 32 * 16)
            elif avg_area > 500:
                cof_pred = cof_pred_3_3.permute(1, 2, 0).reshape(-1, 32 * 9)
            else:
                cof_pred = cof_pred_2_2.permute(1, 2, 0).reshape(-1, 32 * 4)
            ...
            cof_pred = torch.cat((cof_pred, torch.zeros(cof_pred.shape[0], 512 - cof_pred.shape[1]).cuda()), 1)
            mlvl_cofs.append(cof_pred)
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
            mlvl_centerness.append(centerness)
        # print([mlvl_cof.shape for mlvl_cof in mlvl_cofs])
        ...
        if det_bboxes.shape[0] > 0:
            ...
            if avg_area > 1000:
                pos_masks00 = torch.sigmoid(img_mask1 @ det_cofs[:, 0:32].t())
                pos_masks01 = torch.sigmoid(img_mask1 @ det_cofs[:, 32:64].t())
                pos_masks02 = torch.sigmoid(img_mask1 @ det_cofs[:, 64:96].t())
                pos_masks03 = torch.sigmoid(img_mask1 @ det_cofs[:, 96:128].t())
                pos_masks10 = torch.sigmoid(img_mask1 @ det_cofs[:, 128:160].t())
                pos_masks11 = torch.sigmoid(img_mask1 @ det_cofs[:, 160:192].t())
                pos_masks12 = torch.sigmoid(img_mask1 @ det_cofs[:, 192:224].t())
                pos_masks13 = torch.sigmoid(img_mask1 @ det_cofs[:, 224:256].t())
                pos_masks20 = torch.sigmoid(img_mask1 @ det_cofs[:, 256:288].t())
                pos_masks21 = torch.sigmoid(img_mask1 @ det_cofs[:, 288:320].t())
                pos_masks22 = torch.sigmoid(img_mask1 @ det_cofs[:, 320:352].t())
                pos_masks23 = torch.sigmoid(img_mask1 @ det_cofs[:, 352:384].t())
                pos_masks30 = torch.sigmoid(img_mask1 @ det_cofs[:, 384:416].t())
                pos_masks31 = torch.sigmoid(img_mask1 @ det_cofs[:, 416:448].t())
                pos_masks32 = torch.sigmoid(img_mask1 @ det_cofs[:, 448:480].t())
                pos_masks33 = torch.sigmoid(img_mask1 @ det_cofs[:, 480:512].t())

                pos_masks = torch.stack([pos_masks00, pos_masks01, pos_masks02, pos_masks03,
                                          pos_masks10, pos_masks11, pos_masks12, pos_masks13,
                                          pos_masks20, pos_masks21, pos_masks22, pos_masks23,
                                          pos_masks30, pos_masks31, pos_masks32, pos_masks33], dim=0)
                if rescale:
                    pos_masks = self.crop_cuda_2_2(pos_masks,
                                                   det_bboxes[:, :4] * det_bboxes.new_tensor(scale_factor) / scale)
                    # pos_masks = crop_split(pos_masks00, pos_masks01, pos_masks10, pos_masks11, det_bboxes * det_bboxes.new_tensor(scale_factor) / scale)
                else:
                    pos_masks = self.crop_cuda_2_2(pos_masks, det_bboxes[:, :4] / scale)
                ...
                pos_masks01 = torch.sigmoid(img_mask1 @ det_cofs[:, 32:64].t())
                pos_masks02 = torch.sigmoid(img_mask1 @ det_cofs[:, 64:96].t())
                pos_masks10 = torch.sigmoid(img_mask1 @ det_cofs[:, 96:128].t())
                pos_masks11 = torch.sigmoid(img_mask1 @ det_cofs[:, 128:160].t())
                pos_masks12 = torch.sigmoid(img_mask1 @ det_cofs[:, 160:192].t())
                pos_masks20 = torch.sigmoid(img_mask1 @ det_cofs[:, 192:224].t())
                pos_masks21 = torch.sigmoid(img_mask1 @ det_cofs[:, 224:256].t())
                pos_masks22 = torch.sigmoid(img_mask1 @ det_cofs[:, 256:288].t())

                if rescale:
                    pos_masks = torch.stack([pos_masks00, pos_masks01, pos_masks02,
                                             pos_masks10, pos_masks11, pos_masks12,
                                             pos_masks20, pos_masks21, pos_masks22], dim=0)
                    pos_masks = self.crop_cuda_2_2(pos_masks,
                                                   det_bboxes[:, :4] * det_bboxes.new_tensor(scale_factor) / scale)
            ...
            # if rescale:
            #     pos_masks = torch.stack([pos_masks00, pos_masks01, pos_masks10, pos_masks11], dim=0)
            #     pos_masks = self.crop_cuda_2_2(pos_masks, det_bboxes[:, :4] * det_bboxes.new_tensor(scale_factor) / scale)
            #     # pos_masks = crop_split(pos_masks00, pos_masks01, pos_masks10, pos_masks11, det_bboxes * det_bboxes.new_tensor(scale_factor) / scale)
            # else:
            #     pos_masks = torch.stack([pos_masks00, pos_masks01, pos_masks10, pos_masks11], dim=0)
            #     pos_masks = self.crop_cuda_2_2(pos_masks, det_bboxes[:, :4] / scale)
    ...
    def fast_nms(self, boxes, scores, masks, cfg, iou_threshold=0.5, top_k=200):
        ...
        # print(keep.shape, scores.shape, masks.shape)
